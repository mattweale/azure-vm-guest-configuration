...
rm azcopy_linux_amd64_10.14.1.tar.gz
rm -r ./azcopy_linux_amd64_*/azcopy
...
sudo azcopy cp "${CONTAINER}/${IPOPP_TAR_GZ_FILENAME}?${SAS_TOKEN}" "./${IPOPP_TAR_GZ_FILENAME}"
echo 'Downloading DRL-IPOPP_4.1_PATCH_1.tar.gz.......'		
sudo azcopy cp "${CONTAINER}/${PATCH_FILE_NAME}?${SAS_TOKEN}" "./${PATCH_FILE_NAME}"

# Install ipopp
export HOME=/datadrive
sudo tar -C $INSTALL_DIR -xzf $IPOPP_TAR_GZ_FILENAME
sudo chmod -R 755 IPOPP
sudo /datadrive/IPOPP/install_ipopp.sh -installdir $INSTALL_DIR/drl -datadir $INSTALL_DIR/data  -ingestdir $INSTALL_DIR/data/ingest
